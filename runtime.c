...
boolean find_label(Control *ctrl, string s) {
  ...
  for (i = 0; i < EOIL; i++)
    if (!strncmp(token, *(ctrl->instnames_array + i),
      ...
      if (!strcmp(token, *(ctrl->instnames_array + i)))
        add_error(ctrl,
                  "Illegal label: can not be a reserved assembly word (instruction).\n");
      return FALSE;
    }
  ...
  for (i = 0; i < EOCL; i++)
    if (!strcmp(token, *(ctrl->ops_array + i))) {
      add_error(ctrl, "Illegal label: can not be a reserved assembly word (operation).\n");
      return FALSE;
    }
  
  for (i = 0; i < EORL; i++)
    if (!strcmp(token, *(ctrl->regs_array + i))) {
      add_error(ctrl, "Illegal label: can not be a reserved assembly word (register).\n");
      return FALSE;
    }
